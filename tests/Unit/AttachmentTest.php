...
use LaravelCms\Tests\Unit\Engine\CustomAttachmentGenerator;
...
class AttachmentTest extends TestUnitCase
{
    ...
    {
        config()->set('cms.attachment.generator', CustomAttachmentGenerator::class);
        $file = UploadedFile::fake()->create('document.xml', 200);
        $attachment = new File($file, $this->disk);
    ...
    public function testDuplicateAttachmentUpload(): void
    {
        ...
        $upload = (new File($file, $this->disk, rename: "hash"))->load();
        $clone = (new File($clone, $this->disk, rename: "hash"))->load();
    ...
    public function testAttachmentUploadAllowingDuplicate(): void
    {
        ...
        $upload = (new File($file, $this->disk, rename: "hash"))->load();
        $clone = (new File($clone, $this->disk, rename: "hash"))->allowDuplicates()->load();
